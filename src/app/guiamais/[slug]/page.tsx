...
// src/app/guiamais/[slug]/page.tsx
'use client';
          ...
          <div className="mb-8">
            ...
            {!authUser && !authLoading && !allDealsForBusiness.some(d => !d.isVipOffer) && allDealsForBusiness.some(d => d.isVipOffer) && (
                 <Alert variant="default" className="mb-4 bg-accent/10 border-accent/30">
                    ...
                    <AlertDescription>
                        <Link href={`/login?redirect=/guiamais/${slug}`} className="font-semibold underline hover:text-accent/80">Faça login</Link> ou <Link href="/join" className="font-semibold underline hover:text-accent/80">associe-se</Link> para ver se você tem acesso.
                    </AlertDescription>
                </Alert>
            )}
            {!authUser && !authLoading && allDealsForBusiness.some(d => !d.isVipOffer) && (
                 <Alert variant="default" className="mb-4 bg-accent/10 border-accent/30">
                    ...
                    <AlertDescription>
                        <Link href={`/login?redirect=/guiamais/${slug}`} className="font-semibold underline hover:text-accent/80">Faça login</Link> ou <Link href="/join" className="font-semibold underline hover:text-accent/80">associe-se</Link> para usar os benefícios Guia Mais.
                    </AlertDescription>
                </Alert>
            )}
...
export default function BusinessPageWrapper({ params: paramsPromise }: { params: Promise<BusinessPageParams> }) {
    ...
    return (
        <Suspense fallback={<div>Carregando detalhes do estabelecimento...</div>}>
